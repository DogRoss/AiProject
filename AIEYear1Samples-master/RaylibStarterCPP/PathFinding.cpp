...
void PathFinding::FindPath(Vector2 pStartPos, Vector2 pTargetPos)
{
	...
	while (!openList.empty()) {
		Node currentNode = *openList.begin();//Create a node and set it to the first item in the open list
		...
		for (int i = 0; i < grid->GetNeighboringNodes(currentNode, tempList).size(); i++) {	//Loop through each neighbor of the current node

			std::list<Node>::iterator gIterator = tempList.begin();
			advance(gIterator, i);
			bool exists = (std::find(closedList.begin(), closedList.end(), (*gIterator)) == closedList.begin());
			...
			if (moveCost < (*gIterator).gCost || !exists) {//If the f cost is greater than the g cost or it is not in the open list
				...
				if (!exists) {//If the neighbor is not in the openlist
					openList.push_front((*gIterator));//Add it to the list
				}
			}
		}
